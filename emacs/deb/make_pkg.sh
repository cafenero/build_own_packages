...
./autogen.sh
# ./configure --prefix=/home/ubuntu --without-x
./configure --without-x
make
...
cp emacs/lib-src/rcs3log     $PKG_WORK_DIR/usr/local/libexec/emacs/"$EMACS_VERSION"/aarch64-unknown-linux-gnu/
...
cp -r emacs/lisp             $PKG_WORK_DIR/usr/local/share/emacs/"$EMACS_VERSION"/lisp
