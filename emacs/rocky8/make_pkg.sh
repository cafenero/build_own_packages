...
# Build emacs
if [ "$EUID" -eq 0 ]; then
         yum install -y git rpm-build gcc automake ncurses-devel gnutls-devel wget make
else
    sudo yum install -y git rpm-build gcc automake ncurses-devel gnutls-devel wget meke
fi
